...
class MiScraperInstagram:
    def __init__(self, session_id, csrf_token, user_agent=None):
        ...
        # Asegurar que las cookies de autenticación viajen con cada request
    ...
    def get_user_id_from_username(self, username):
        ...
        try:
            ...
            if response.status_code == 429:
                print(f"⚠️  Límite de tasa alcanzado. Esperando 30s...")
                time.sleep(30)
                return self.get_user_id_from_username(username)

            if response.status_code != 200:
                print(f"❌ HTTP {response.status_code} para username {username}")
                return None
            ...
            print(f"❌ Error obteniendo user_id para @{username}: {e}")
            return None
    ...
    def get_following(self, user_id, max_users=None, batch_delay=5, individual_delay=8):
        ...
        while has_more and (max_users is None or len(all_profiles) < max_users):
            ...
            try:
                ...
                if response.status_code == 429:
                    print("⚠️  Límite de tasa alcanzado. Esperando 60s...")
                    time.sleep(60)
                    continue
                ...
                for user in users:
                    ...
                    print(f"[{len(all_profiles) + 1}/{max_users or '?'}] Obteniendo @{username}...", end=" ", flush=True)
                ...
                # Algunas respuestas no incluyen has_more; seguimos mientras llegue next_max_id o big_list
                has_more = bool(next_max_id) or data.get('big_list', False)
...
def main():
    print("=" * 60)
    print("MI SCRAPER DE INSTAGRAM v1.0")
    print("=" * 60)
    print()

    print("⚠️  REQUISITOS:")
    print("   - Sesión válida en Instagram")
    print("   - Username o User ID numérico del perfil objetivo")
    print()

    # Datos de entrada (objetivo y sesión actual)
    username_or_id = input("👤 Username o User ID del objetivo: ").strip()
    session_id = input("🔑 SessionID (cuenta logueada): ").strip()
    csrf_token = input("🛡️  CSRF Token: ").strip()

    scraper = MiScraperInstagram(session_id, csrf_token)

    # Resolver user_id del objetivo
    if username_or_id.isdigit():
        user_id = username_or_id
        print(f"✅ Usando User ID directo: {user_id}")
    else:
        user_id = scraper.get_user_id_from_username(username_or_id)
        if not user_id:
            print(f"❌ No se pudo obtener el user_id para @{username_or_id}")
            return
        print(f"✅ User ID obtenido para @{username_or_id}: {user_id}")
